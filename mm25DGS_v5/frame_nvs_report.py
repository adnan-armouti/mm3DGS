...
Collects every ``results.json`` under ``mm25DGS_v5/output_frame_nvs/`` and
...
``mm25DGS_v5.rasterizer.reparameterize_torch``. Surface normals from
...
OUTPUT_ROOT = os.path.join(PROJECT_ROOT, 'mm25DGS_v5', 'output_frame_nvs')
...
def build_report(rows):
    ...
    lines.append('')
    lines.append('*Generated by `mm25DGS_v5.frame_nvs_report`.*')
    lines.append('')
    ...
    if not rows:
        lines.append('*(no results.json files under `mm25DGS_v5/output_frame_nvs/`)*')
        lines.append('')
        return '\n'.join(lines) + '\n'
    ...
    lines.append('Every run saves its best-iter model state to '
                 '`mm25DGS_v5/output_frame_nvs/<scene>_<tag>/best_model.pt`. '
    ...
    lines.append('To recover physical material values: '
                 '`physics = mm25DGS_v5.rasterizer.reparameterize_torch(raw_materials)` '
