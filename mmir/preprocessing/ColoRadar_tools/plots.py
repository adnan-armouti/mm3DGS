...
if __name__ == '__main__':
  ...
  print(f"len(radar_indices): {len(radar_indices)}")
  print(f"len(lidar_indices): {len(lidar_indices)}")
  ...
  while radar_idx < len(radar_indices) or lidar_idx < len(lidar_indices):
    if radar_idx < len(radar_indices) and radar_timestamps[radar_indices[radar_idx]] < lidar_timestamps[lidar_indices[lidar_idx]]:
      if (radar_idx == 182):
        print("*"*50)
        print(f"radar_idx: {radar_idx}" )
        print(f"radar_indices[radar_idx]: {radar_indices[radar_idx]}" )
        print(f"radar_timestamps[radar_indices[radar_idx]]: {radar_timestamps[radar_indices[radar_idx]]}")
        print(f"lidar_idx: {lidar_idx}" )
        print(f"lidar_indices[lidar_idx]: {lidar_indices[lidar_idx]}" )
      plot_data.append((radar_timestamps[radar_indices[radar_idx]],
                        radar_gt[radar_idx],
                        radar_indices[radar_idx],
                        'radar'))
      if (radar_idx == 182):
        print(f"len(plot_data): {len(plot_data)}")
        print("*"*50)
      radar_idx += 1
