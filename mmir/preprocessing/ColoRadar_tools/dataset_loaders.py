...
def get_pointcloud(index, seq_dir, params):
  if params['sensor_type'] == 'lidar':
    filename = os.path.join(seq_dir, 'lidar/pointclouds/lidar_pointcloud_' + str(index) + '.bin')
  else:
    filename = os.path.join(seq_dir, 'single_chip/pointclouds/data/radar_pointcloud_' + str(index) + '.bin')
...
def get_timestamps(seq_dir, params):
  if params['sensor_type'] == 'lidar': 
    filename = os.path.join(seq_dir, 'lidar/timestamps.txt')
  elif params['sensor_type'] == 'imu':
    filename = os.path.join(seq_dir, 'imu/timestamps.txt')
  elif params['sensor_type'] == 'groundtruth':
    filename = os.path.join(seq_dir, 'groundtruth/timestamps.txt')
  else:
    if params['sensor_type'] == 'cascade':
      filename = os.path.join(seq_dir, 'cascade')
    else:
      filename = os.path.join(seq_dir, 'single_chip')
...
def read_phase_freq_calib(calib_filename):
  ...
  num_tx = lines["antennaCalib"]["numTx"]
  num_rx = lines["antennaCalib"]["numRx"]
  frequency_slope = lines["antennaCalib"]["frequencySlope"]
  sampling_rate = lines["antennaCalib"]["samplingRate"]

  # phase_cal_mat_str = np.asarray(lines["antennaCalib"]["frequencyCalibrationMatrix"], dtype="float")
  phase_cal_mat_arr = np.asarray(lines["antennaCalib"]["phaseCalibrationMatrix"], dtype="float")
  phase_cal_mat_arr = phase_cal_mat_arr[:-1:2] + 1j * phase_cal_mat_arr[1::2]
  phase_cal_mat = phase_cal_mat_arr.reshape(num_tx, num_rx)
  ...
  freq_cal_mat_arr = np.asarray(lines["antennaCalib"]["frequencyCalibrationMatrix"], dtype="float")
  freq_cal_mat = freq_cal_mat_arr.reshape(num_tx, num_rx)
