...
from figures.fig_common_highres import (BACKGROUND_COLOR, FIG_WIDTH_INCHES,
...
def _ax_at(fig, left, bot, w, h, **kwargs):
    ...
    Fix #6 (FIGURE_CHANGES.md §6): hide the axes patch entirely. set_alpha(0)
    still emits a transparent rect path which some PDF viewers render as a
    faint outline.
    """
    fw, fh = fig.get_size_inches()
...
def _draw_thumbnail(fig, ax, render_fn, fallback_text="Image"):
    try:
        ...
        if arr.ndim == 3:
            ...
            # (border crop uses the < 245 mask — Fix #5 caveat: this is the
            # CROP threshold and is fine; the bg-snap threshold below uses
            # >= 250 instead, which is the critical one).
            ...
            # Fix #5 (FIGURE_CHANGES.md §5): snap ONLY very-near-white pixels
            # (>= 250 on every channel) to the figure tile color. With
            # defaultLit shading, lit mesh highlights reach 200-249;
            # >= 200 would erase those highlights. Use the section column-tile
            # color so the snap blends with the tile, not the figure card.
            tile_rgb = np.array([235, 235, 235], dtype=np.uint8)  # TILE_COLOR
            light = np.all(arr[:, :, :3] >= 250, axis=2)
            arr[light] = tile_rgb
        # Fix #4 (FIGURE_CHANGES.md §4): figimage_in_axes places the raster
        # at native pixel density (Lanczos-resized to panel) instead of
        # imshow's panel-pixel resample, so 3D-mesh thumbnails stay crisp at
        # high zooms.
        figimage_in_axes(fig, arr, ax, figure_bg="#ebebeb")
...
def _draw_section1(fig):
    ...
    if os.path.exists(panel_path):
        try:
            ...
            for sp in ax.spines.values():
                sp.set_visible(False)
            # Fix #4 (FIGURE_CHANGES.md §4): figimage_in_axes for the panel_a
            # 3DPS scene render. The axes was already sized to match the
            # image's aspect (img_axes_w = img_h * img_aspect), so fit="equal"
            # naturally fills the panel — fit="auto" with the max_dim_px cap
            # would shrink the image below the panel size.
            figimage_in_axes(fig, arr, ax, fit="equal", figure_bg="#ebebeb")
        except Exception as e:
...
def _draw_prediction_section(fig, sec_left, sec_w, adc_path, ra_path,
    ...
    try:
        ...
        # Fix #4: figimage_in_axes for the CRP heatmap PNG.
        figimage_in_axes(fig, arr, ax_adc, fit="equal", figure_bg="#ebebeb")
    except Exception as e:
    ...
    try:
        ...
        # Fix #4: figimage_in_axes for the |RA| heatmap PNG.
        figimage_in_axes(fig, ra_img, ax_ra, fit="equal", figure_bg="#ebebeb")
    except Exception as e:
...
def generate_figure(output_dir, center_variant="system_overview",
    ...
    # Fix #4 (FIGURE_CHANGES.md §4 + §7): bump fig.dpi so panel pixel grids
    # support high-fidelity figimage embedding. Size off the LARGEST figimage
    # panel — Section 1's panel_a (~1.1" wide) — so its panel pixel size is
    # close to max_dim_px (=2000) without exceeding it. Smaller panels (CRP/RA
    # at SPLAT_H/2 ≈ 0.5") then fit comfortably below the cap and fill their
    # axes box correctly.
    largest_figimage_panel_in = 1.1
    fig_dpi = needed_dpi_for_native_embed(
        [], largest_figimage_panel_in, largest_figimage_panel_in)
    ...
    # Fix #7 (FIGURE_CHANGES.md §7): savefig dpi tracks fig.dpi exactly,
    # required for figimage panel-pixel offsets to come out right.
