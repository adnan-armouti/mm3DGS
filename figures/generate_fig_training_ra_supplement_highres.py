...
def _draw_tile(ax, ra: Optional[np.ndarray], corr: Optional[float], cmap="hot"):
    ...
    if ra is None:
        # Hide the axes patch on missing tiles too — the red column highlight
        # behind 'missing' tiles in the test column reads better without a
        # phantom rect outline.
        ax.patch.set_visible(False)
        ax.text(0.5, 0.5, "missing",
                ha="center", va="center", transform=ax.transAxes,
                fontsize=FIGURE_SMALL_PT, color="#999999")
        return
    # Fix #6 (FIGURE_CHANGES.md §6): pcolormesh + rasterized=True instead of
    # imshow, so the heatmap collapses to a single raster image at savefig
    # DPI without per-cell tiling lines.
    ax.pcolormesh(ra_cart_to_linear(ra), cmap=cmap,
                  vmin=0.0, vmax=1.0, rasterized=True,
                  shading="flat", linewidth=0, edgecolors="none",
                  antialiased=False)
    ax.set_aspect("equal")
    # Fix #6: hide axes patch so it doesn't outline against the rounded card.
    ax.patch.set_visible(False)
    if corr is not None:
...
def generate_one_scene(scene: str, test_frame: int,
    ...
    # Fix #4: bump fig.dpi so each panel rasterizes at ~max_dim_px on its
    # longer side. With 9 cols x ~0.7" panels this lands in the 2800-3000 range.
    fig_dpi = needed_dpi_for_native_embed(
        [], layout.cell_w_in, layout.cell_h_in,
    )

    fig = plt.figure(figsize=(layout.fig_w, layout.fig_h),
                     facecolor=BACKGROUND_COLOR, dpi=fig_dpi)
    add_rounded_bg(fig)
    ...
    fig.savefig(output_path, dpi=save_dpi, facecolor="none",
                 edgecolor="none", bbox_inches=None)
    fig.savefig(output_path.replace(".pdf", ".png"), dpi=save_dpi,
