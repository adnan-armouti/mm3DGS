...
from figures.fig_common_highres import (
...
def _imshow(fig, image_path, x_in, y_in, w_in, h_in, fig_w, fig_h,
             ...
             figure_bg="#ECEBEB"):
    """Place an image in axes at the given inch coordinates.

    Fix #4 (FIGURE_CHANGES.md §4): when clip_path is None, use
    figimage_in_axes so the raster is embedded at native pixel resolution
    (Lanczos-resized to the panel and capped at DEFAULT_FIGIMAGE_MAX_DIM_PX).
    figimage doesn't support clipping, so when a rounded-corner clip is
    requested we fall back to imshow + set_clip_path. The high fig.dpi we
    use means imshow's panel-pixel resample is still high-resolution.
    Fix #6: hide axes patch instead of set_facecolor("none")+set_alpha(0).
    """
    ...
    if clip_path is None:
        # Fast path: figimage_in_axes places the raster natively. fit must
        # match the imshow aspect parameter ("auto" = stretch, else "equal").
        # Forward zorder explicitly: fig.figimage default zorder=0, but the
        # teaser draws colored row/sub-tile patches at zorder>=1 over the
        # figure bg, which would otherwise cover the embedded raster.
        fit = "auto" if aspect == "auto" else "equal"
        figimage_in_axes(fig, img, ax, fit=fit, figure_bg=figure_bg,
                          zorder=zorder)
    else:
        # Clipped path: keep imshow so we can apply set_clip_path. Native
        # imshow at the bumped fig.dpi still gives high-resolution output.
...
def _draw_nvs_column(fig, x0_in, y0_in, w_in, h_in, fig_w, fig_h, panel_dir,
    ...
    # Fix #6: hide axes patch entirely.
    ax.patch.set_visible(False)
...
def generate(args):
    ...
    # panel — the compression-column panels (~1.0" sq) — so their panel pixel
    # size is close to max_dim_px (=2000) without exceeding it. Smaller panels
    # (NVS sub-tiles ~0.34" sq, rendering cells ~0.34" sq) fit comfortably
    # below the cap and fill their axes box correctly. Sizing off the
    # smallest panel previously made the largest panel exceed max_dim_px,
    # which made the embedded raster center with margins inside the panel.
    largest_figimage_panel_in = 1.0
    ...
    # Fix #6 / Fix #2: hide the figure rectangle entirely.
    ...
    fig.savefig(out_pdf, dpi=save_dpi, facecolor="none", edgecolor="none")
