...
def _draw_ra(ax, ra_cart, ra_corr=None):
    ...
    Fix #6 (FIGURE_CHANGES.md §6): pcolormesh with rasterized=True,
    shading="flat", linewidth=0, edgecolors="none", antialiased=False.
    Mirrors the reference recipe so the heatmap collapses to a single raster
    image at savefig DPI (no per-cell tiling lines).
    """
    ax.pcolormesh(
...
def _draw_placeholder(ax, msg):
    ...
    The dark facecolor is intentional (signals 'missing'); we keep it as a
    visible facecolor here rather than a separate patch for simplicity.
    """
...
def _frame_axes(ax, is_placeholder=False):
    """Hide ticks/spines. Fix #6: use ax.patch.set_visible(False) on real
    heatmap cells so the axes' own rectangle path doesn't render as a faint
    outline on top of the rounded card. Placeholder cells keep their dark
    facecolor visible."""
    ax.set_xticks([])
    ax.set_yticks([])
...
def generate_figure(scenes, baselines_dir, ours_dir, output_dir):
    ...
    # Fix #4 (FIGURE_CHANGES.md §4 + §7): bump fig.dpi so each panel's pixel
    # footprint matches DEFAULT_FIGIMAGE_MAX_DIM_PX. With pcolormesh
    # rasterized=True, each heatmap is rendered to that pixel grid at savefig
    # time, giving ~20x zoom headroom on a 1" panel.
    fig_dpi = needed_dpi_for_native_embed(
        [], layout.cell_w_in, layout.cell_h_in,
    )
    print(f"  fig.dpi={fig_dpi} (cell={layout.cell_w_in:.2f}\")")

    fig = plt.figure(figsize=(layout.fig_w, layout.fig_h), dpi=fig_dpi)
    add_rounded_bg(fig)
    ...
    for col_idx, scene in enumerate(ranked):
        for row_idx, row_key in enumerate(ROW_KEYS):
            ...
            _frame_axes(ax, is_placeholder=is_placeholder)
    ...
    # Fix #7 (FIGURE_CHANGES.md §7): savefig dpi tracks fig.dpi.
    save_dpi = fig.get_dpi()
    fig.savefig(out_pdf, dpi=save_dpi, facecolor="none", edgecolor="none")
    fig.savefig(out_png, dpi=save_dpi, facecolor=BACKGROUND_COLOR)
    plt.close(fig)
    print(f"\nSaved: {out_pdf}\nSaved: {out_png}")
    return out_pdf
