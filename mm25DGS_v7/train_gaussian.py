"""mm25DGS_v5 — c6 hemisphere integral training (point-based primitive).
...
Usage:
  CUDA_VISIBLE_DEVICES=0 python -m mm25DGS_v5.train_gaussian --scene seq_0_frame_135 --iters 500
  CUDA_VISIBLE_DEVICES=0 python -m mm25DGS_v5.train_gaussian --all --iters 500
"""
...
from mm25DGS_v5.load_pretrained import (
    load_trained_config, load_pattern_data, TRAIN_OUTPUT_DIR, SCENES,
)
from mm25DGS_v5.material_diagnostics import MaterialDiagnostics
...
def render_gaussians(model, rast, vertex_areas, active_mask=None,
    ...
    """
    from mm25DGS_v7.rasterizer_factorized import render_factorized
...
def render_gaussians_doppler(model, rast, v_ego, vertex_areas,
                              active_mask=None, shadow_mask=None,
                              detach_phase=True, bsdf_mode='full',
                              disabled_components=None, n_chirps=16):
    """v7 Doppler-aware range-profile renderer wrapper.

    ``v_ego``: (3,) world-frame ego velocity tensor (FROZEN; not
    learnable per md/mm25dgs_v7_doppler_plan.md §5).

    Returns ``(rp_real, rp_imag)`` of shape
    ``(n_chirps=16, n_tx=12, n_rx=16, K=256)`` complex — per-chirp
    complex range profiles with analytic Doppler + TDM phase
    modulation applied.
    """
    ...
    return render_factorized_doppler(
        positions, normals, areas, raw_materials, rast,
        reparameterize_torch,
        v_ego=v_ego, n_chirps=n_chirps,
...
def train_gaussians(scene, num_iters=500, target_n=90000, verbose=True,
    ...
    if freeze_mat_cols is not None and len(freeze_mat_cols) > 0:
        ...
        if verbose:
            from mm25DGS_v5.material_diagnostics import PARAM_NAMES
    ...
    # Save outputs
    output_dir = os.path.join(PROJECT_ROOT, 'mm25DGS_v5', 'output', scene)
