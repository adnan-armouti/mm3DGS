...
Per scene the driver runs:
  ...
  1. **Pass 1** — per-frame independent alignment (v5-CUDA renderer-2-DOF
     ...
     ``mm25DGS_v5_v4.preprocessing.alignment.cascaded_alignment.run_all``
  ...
  2. **Pass 2** — trajectory-aware re-alignment of pass-1 outliers
     ...
     ``mm25DGS_v5_v4.preprocessing.alignment.pass2.run_pass2.run_pass2_for_scene``
...
-----
    ...
    python -m mm25DGS_v5_v4.preprocessing.alignment.run_alignment --all

    # Single scene
    python -m mm25DGS_v5_v4.preprocessing.alignment.run_alignment \
...
def run_pass1(scene, data_root, output_root, target_n=30000,
    ...
    from mm25DGS_v5_v4.preprocessing.alignment.cascaded_alignment import run_all
    return run_all(
        data_root=data_root,
...
def run_pass2(scene, data_root, prior_weight, gate_mad, target_n, verbose=True):
    ...
    if mi.variant() is None:
        mi.set_variant('cuda_ad_rgb')
    from mm25DGS_v5_v4.preprocessing.alignment.pass2.run_pass2 import (
        run_pass2_for_scene,
    )
...
def run_pass3(scene, data_root, anchor_source, prior_weight, gate_margin,
    ...
    if mi.variant() is None:
        mi.set_variant('cuda_ad_rgb')
    from mm25DGS_v5_v4.preprocessing.alignment.per_chirp_alignment import (
