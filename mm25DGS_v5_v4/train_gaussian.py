"""mm25DGS_v5_v4 — c6 hemisphere integral training (point-based primitive).
...
Usage:
  CUDA_VISIBLE_DEVICES=0 python -m mm25DGS_v5_v4.train_gaussian --scene seq_0_frame_135 --iters 500
  CUDA_VISIBLE_DEVICES=0 python -m mm25DGS_v5_v4.train_gaussian --all --iters 500
"""
...
from mm25DGS_v5_v4.rasterizer import (
...
from mm25DGS_v5_v4.material_diagnostics import MaterialDiagnostics
...
# =========================================================================
# v5_v4 Phase 2 — radar-aware init (Direction A from
# md/mm25dgs_v5_v4_smart_sampling_and_densification.md)
# =========================================================================

_INIT_VARIANTS = {
    'baseline':  '(unchanged) v5 init: FOV+RX+seed-pose-cosine resample + FPS',
    'A1_no_fps': 'baseline minus FPS (pure random sample of post-resample pool)',
    'A2_union_cos':       'union over train poses, weight = sum_F max(0, cos θ_i)',
...
def _per_pose_amplitude_score(xyz, normals, pose_dict_list,
                                ...
                                device='cuda'):
    """Per-point sum-of-amplitudes score across a list of pose dicts.

    Each pose dict has 'rx_positions' (n_rx,3) and 'tx_boresights' (n_tx,3)
    tensors (the same format _build_frame_poses returns).

    Returns: numpy array of shape (N,) with non-negative score per point.
    Score(p) = Σ_F  visible(p,F) · max(0,cos θ_i)^cos · (1/d²)^invd2 · FOV(p,F)

    Visibility uses Mitsuba ray test against the same scene rast loads.
    """
    N = len(xyz)
    score = np.zeros(N, dtype=np.float64)

    xyz_np = np.asarray(xyz, dtype=np.float32)
    nrm_np = np.asarray(normals, dtype=np.float32)
    nrm_np = nrm_np / np.maximum(
        np.linalg.norm(nrm_np, axis=1, keepdims=True), 1e-12)

    for pose in pose_dict_list:
        rx_center = pose['rx_positions'].mean(dim=0).cpu().numpy()
        boresight = pose['tx_boresights'].mean(dim=0).cpu().numpy()
        boresight = boresight / max(np.linalg.norm(boresight), 1e-8)

        delta = xyz_np - rx_center                               # (N, 3)
        dist  = np.linalg.norm(delta, axis=1).clip(min=1e-6)     # (N,)
        dir_to_point = delta / dist[:, None]                     # (N, 3)
        cos_bore = (dir_to_point * boresight).sum(axis=-1)       # (N,)
        ...
        # cos θ_i (incidence on the surface; double-sided abs since we
        # don't track which side the surface is from)
        cos_i = np.abs((nrm_np * (-dir_to_point)).sum(axis=-1))  # (N,)

        # Visibility raycast at this pose
        if check_visibility and mi_scene is not None and in_fov.sum() > 0:
            visible_idx = np.flatnonzero(in_fov)
            visible_pts = xyz_np[visible_idx]
            vis = _ray_test_visibility_batched(
                visible_pts, rx_center, mi_scene)
            visible_mask = np.zeros(N, dtype=bool)
            visible_mask[visible_idx[vis]] = True
        else:
            visible_mask = in_fov.copy()

        amp = visible_mask.astype(np.float64)
        if use_cos:
            amp = amp * np.maximum(cos_i, 0.0)
        if use_invd2:
            amp = amp * (1.0 / (dist ** 2))
        score += amp
...
def init_visible_weighted_radar_aware(
    ...
    Replaces v5's seed-pose-only FOV+cosine+FPS with union-amplitude
    importance sampling across all train poses, optionally weighted by
    LiDAR intensity (column 6 of pcl.npy).

    See md/mm25dgs_v5_v4_smart_sampling_and_densification.md §2 for the
    full motivation + ablation table (A1-A5 variant names).

    Args:
      scene: scene name (used to load pcl.npy via rast._mesh_file).
      rast: existing Rasterizer (used for FOV / max-range / mesh).
      train_poses_chirp0: list of pose dicts (one per train frame),
        each with 'rx_positions' / 'tx_boresights' as
        ``_build_frame_poses(...)[0]`` returns at chirp 0.
      target_n: target Gaussian count (e.g. 20K).
      variant: one of _INIT_VARIANTS keys.
      cos_bore_min: per-pose FOV-cone threshold.
      fps_oversample_factor: only used for A5 (FPS to 1.5×N then resample).
      intensity_alpha: LiDAR-intensity prior is `intensity ** alpha` (A4/A5).
        alpha=1 raw, alpha=0.5 sqrt-soften, alpha=0 disables.
      verbose: print init pipeline stages.

    Returns: PointPrimitives model (or tuple per return_pool).
    """
    assert variant in _INIT_VARIANTS, f'unknown variant {variant}'
    ...
    nrm_full = nrm_full / np.maximum(
        np.linalg.norm(nrm_full, axis=1, keepdims=True), 1e-12)
    intensity_full = (pcl[:, 6].astype(np.float32)
                       if pcl.shape[1] >= 7 else
                       np.ones(len(pcl), dtype=np.float32))
    if verbose:
        print(f"  [v5_v4 init/{variant}] pcl: {len(xyz_full):,} points, "
              f"7 cols={pcl.shape[1] >= 7}")

    # Loose seed-pose FOV pre-cull (just to keep the per-pose union
    # work tractable). Use a generous cos_bore_min so we don't kill
    # candidates that any train pose could see.
    rx_center_seed = rast.rx_positions.mean(dim=0).cpu().numpy()
    boresight_seed = rast.tx_boresights.mean(dim=0).cpu().numpy()
    boresight_seed = boresight_seed / max(np.linalg.norm(boresight_seed), 1e-8)
    delta = xyz_full - rx_center_seed
    dist = np.linalg.norm(delta, axis=1).clip(min=1e-6)
    cos_bore = ((delta / dist[:, None]) * boresight_seed).sum(axis=-1)
    max_range = rast.K * 299792458.0 / (2.0 * rast.slope * (rast.K / rast.sample_rate))
    pre_mask = (cos_bore > -0.2) & (dist > 1.5) & (dist < max_range * 1.1)
    xyz_pre = xyz_full[pre_mask]
    nrm_pre = nrm_full[pre_mask]
    int_pre = intensity_full[pre_mask]
    if verbose:
        print(f"  [v5_v4 init/{variant}] after loose pre-cull: {len(xyz_pre):,} points")

    # Load Mi scene for visibility raycasts (multi-pose)
    rast.load_mi_scene()

    if variant == 'baseline':
        # Fall back to v5 init via the existing function
    ...
    if variant == 'A1_no_fps':
        # Same as baseline (seed-pose cosine resample) but no FPS, just
        # random sample of the resampled pool. Implementation: do FOV +
        # RX visibility + seed cos resample as in baseline, then random.
        n_intermediate = max(200000, 3 * target_n)
        # Use single-pose path (seed pose only)
        score = _per_pose_amplitude_score(
            xyz_pre, nrm_pre,
            [{'rx_positions': rast.rx_positions, 'tx_boresights': rast.tx_boresights}],
            use_cos=True, use_invd2=False,
            cos_bore_min=cos_bore_min, max_range=max_range,
            mi_scene=rast._mi_scene, check_visibility=True, device=device)
        n_resample = min(n_intermediate, 3 * target_n)
        probs = score / max(score.sum(), 1e-12)
        rng = np.random.default_rng(42)
        sampled_idx = rng.choice(len(xyz_pre), size=n_resample, replace=True, p=probs)
        unique_idx = np.unique(sampled_idx)
        # Then random (no FPS) subsample to target_n
        if len(unique_idx) > target_n:
            sel = rng.choice(len(unique_idx), size=target_n, replace=False)
            unique_idx = unique_idx[sel]
        xyz = xyz_pre[unique_idx]
        normals = nrm_pre[unique_idx]
    else:
        # A2/A3/A4/A5 — union over train poses
        use_cos    = True
        use_invd2  = (variant in ('A3_union_amplitude', 'A4_union_amp_lidar', 'A5_amp_lidar_fps'))
        score = _per_pose_amplitude_score(
            xyz_pre, nrm_pre, train_poses_chirp0,
            use_cos=use_cos, use_invd2=use_invd2,
            cos_bore_min=cos_bore_min, max_range=max_range,
            mi_scene=rast._mi_scene, check_visibility=True, device=device)
        if variant in ('A4_union_amp_lidar', 'A5_amp_lidar_fps') and intensity_alpha > 0:
            score = score * (np.maximum(int_pre, 1.0) ** intensity_alpha)

        n_visible = int((score > 0).sum())
        if verbose:
            print(f"  [v5_v4 init/{variant}] points with score > 0 (visible from "
                  f"≥1 train pose): {n_visible:,}")

        if n_visible < target_n:
            # not enough genuinely visible points — pad by uniform sample
            print(f"    !! only {n_visible} visible — padding with uniform sample")

        # Sample without replacement, weighted by score.
        rng = np.random.default_rng(42)
        nz = score > 0
        if nz.sum() >= target_n:
            probs = score[nz] / score[nz].sum()
            cand_idx = np.flatnonzero(nz)
            sel = rng.choice(len(cand_idx), size=target_n, replace=False, p=probs)
            chosen = cand_idx[sel]
        else:
            # take all visible + uniform fill from the rest
            visible_idx = np.flatnonzero(nz)
            rest_idx = np.flatnonzero(~nz)
            n_extra = target_n - len(visible_idx)
            extra = rng.choice(rest_idx, size=n_extra, replace=False)
            chosen = np.concatenate([visible_idx, extra])
        xyz = xyz_pre[chosen]
        normals = nrm_pre[chosen]

        # A5: oversample by 1.5×, then FPS-down to target_n
        if variant == 'A5_amp_lidar_fps':
            over_n = int(target_n * fps_oversample_factor)
            if nz.sum() >= over_n:
                # Re-sample over_n with replacement to bias by score, then FPS
                idx2 = rng.choice(len(cand_idx), size=over_n, replace=True, p=probs)
                idx2 = np.unique(idx2)
                # FPS down to target_n
                if len(idx2) > target_n:
                    pts = torch.from_numpy(xyz_pre[cand_idx[idx2]]).to(device)
                    sel2 = _farthest_point_sampling(pts, target_n).cpu().numpy()
                    chosen = cand_idx[idx2[sel2]]
                    xyz = xyz_pre[chosen]
                    normals = nrm_pre[chosen]
...
def train_gaussians(scene, num_iters=500, target_n=90000, verbose=True,
    ...
    if freeze_mat_cols is not None and len(freeze_mat_cols) > 0:
        ...
        if verbose:
            from mm25DGS_v5_v4.material_diagnostics import PARAM_NAMES
    ...
    # Save outputs
    output_dir = os.path.join(PROJECT_ROOT, 'mm25DGS_v5_v4', 'output', scene)
